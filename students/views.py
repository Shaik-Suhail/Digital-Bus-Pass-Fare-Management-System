...
def renew_pass(request, application_number):
    if 'email' not in request.session:
        messages.error(request, "You need to be logged in to renew your pass.")
        return redirect('student')

    try:
        old_selection = RouteSelection.objects.get(application_number=application_number)
    except RouteSelection.DoesNotExist:
        messages.error(request, "Application not found.")
        return redirect('userhome')

    # Extend pass based on previous type
    if old_selection.pass_type == 'Monthly':
        new_validity = timezone.now() + timedelta(days=30)
        new_fare = old_selection.route.fare
    elif old_selection.pass_type == 'Quarterly':
        new_validity = timezone.now() + timedelta(days=90)
        new_fare = old_selection.route.fare * 3
    elif old_selection.pass_type == 'Annual':
        new_validity = timezone.now() + timedelta(days=365)
        new_fare = old_selection.route.fare * 12

    # Generate new application number
    new_application_number = f"APP-{uuid.uuid4().hex[:8].upper()}"

    # Create new RouteSelection
    renewed_selection = RouteSelection.objects.create(
        applicant=old_selection.applicant,
        route=old_selection.route,
        pass_type=old_selection.pass_type,
        application_number=new_application_number,
        fare_amount=new_fare,
        pass_valid_upto=new_validity,
        pass_applied_date=timezone.now(),
    )

    # Generate QR code
    qr_data = f"Application Number: {new_application_number}\nName: {old_selection.applicant.name}\nExpires: {new_validity}\nRoute: {old_selection.route.start_point} to {old_selection.route.end_point}"
    qr_image = qrcode.make(qr_data)
    buffer = BytesIO()
    qr_image.save(buffer, format='PNG')
    renewed_selection.qr_code.save(f"{new_application_number}.png", ContentFile(buffer.getvalue()), save=False)
    renewed_selection.save()

    messages.success(request, f"Renewal successful. New application number: {new_application_number}")
    return redirect('display_amount', route_selection_id=renewed_selection.id)
